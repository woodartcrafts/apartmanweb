...
const ChargeBulkPage = lazy(() =>
  import("./admin/ChargeBulkPage").then((module) => ({
    default: module.ChargeBulkPage,
  }))
);
const ExpenseEntryPage = lazy(() =>
  import("./admin/ExpenseEntryPage").then((module) => ({
    default: module.ExpenseEntryPage,
  }))
);
const AuditLogsPage = lazy(() =>
  import("./admin/AuditLogsPage").then((module) => ({
...
const MeetingGuidePage = lazy(() =>
  import("./admin/MeetingGuidePage").then((module) => ({
...
function AdminPage({ user, onSessionExpired }: { user: LoginResponse["user"] | null; onSessionExpired: () => void }) {
  ...
  useEffect(() => {
    ...
    // firstAccessibleAdminPath null ise hiÃ§ eriÅŸilebilir sayfa yok;
    // /login'e yÃ¶nlendirmek sonsuz dÃ¶ngÃ¼ye girer (kullanÄ±cÄ± giriÅŸli â†’ tekrar reports'a dÃ¶n).
  ...
  const bankPreviewPreSaveWarnings = useMemo(() => {
    ...
    for (const item of bankPreviewRowsWithFlags) {
      ...
      const hasLetters = /[a-zA-ZÄ±ÄŸÃ¼ÅŸÃ¶Ã§Ä°ÄÃœÅÃ–Ã‡]/.test(description);
  ...
  function formatBulkStatementDescription(description?: string | null): string {
    ...
    const title = parts[0] ?? "";
    const invoiceAmountPart = parts.find((part) => /^fatura\s+tutar[Ä±i]\s*:/i.test(part));
    const invoiceDatePart = parts.find((part) => /^fatura\s+tarih[Ä±i]\s*:/i.test(part));
    ...
    if (isDogalgazTitle && invoiceDatePart) {
      const rawValue = invoiceDatePart.replace(/^fatura\s+tarih[Ä±i]\s*:/i, "").trim();
  ...
  function formatAccountingStatementDescription(description?: string | null): string {
    ...
    const rawParts = description
      ...
      .map((x) => x.trim())
      .filter(Boolean)
      .filter((part) => !/^baslang[Ä±i]c\s*:/i.test(part) && !/^bitis\s*:/i.test(part) && !/^door\s*:/i.test(part));
    ...
    const invoiceDatePart = sourceParts.find((part) => /^fatura\s+tarih[Ä±i]\s*:/i.test(part));
    const isDogalgazTitle = /^dogalgaz$/i.test(title.replace(/\s+/g, " ").trim());
    ...
    const parts = sourceParts
      .map((part) => {
        const invoiceDateValue = part.match(/^fatura\s+tarih[Ä±i]\s*:\s*(.+)$/i)?.[1]?.trim();
        if (invoiceDateValue) {
        ...
        <Route
          ...
          element={
            <section className="dashboard report-page">
              ...
              {/* â”€â”€â”€ Filter Card â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
              ...
              {/* â”€â”€â”€ Kod Dagilimi / Hizli Filtre â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
              ...
              {/* â”€â”€â”€ Muaf Daireler â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
              ...
              {/* â”€â”€â”€ Sonuc Tablosu â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
            ...
            <section className="dashboard expense-dist-page">
              <div className="card admin-form expense-dist-form-surface">
                ...
                <div className="expense-dist-section-head">
                  <h4 className="sheet-section-title">ğŸ§¾ Fatura Bilgileri</h4>
                ...
                <div className="expense-dist-section-head">
                  <h4 className="sheet-section-title">âš–ï¸ Katsayi Dagitim Secimi</h4>
                ...
                <div className="expense-dist-section-head">
                  <h4 className="sheet-section-title">ğŸ”§ Yuvarlama</h4>
            ...
            (() => {
              ...
              const inferredInvoiceDateFromDescription = (() => {
                ...
                const part = criteria.description
                  .split("|")
                  .map((x) => x.trim())
                  .find((x) => /^fatura\s+tarih[Ä±i]\s*:/i.test(x));
                if (!part) {
                  return null;
                }
                const raw = part.replace(/^fatura\s+tarih[Ä±i]\s*:/i, "").trim();
                if (!raw) {
                  return null;
                }
